...
# ==============================================================================
# FIGURE 4F-G: Collapsed Bubble Plots with Numbered Annotations & Tables
# Single panel per season (no BP/MF/CC facets), numbered labels with legend table
# ==============================================================================

cat("\n==============================================================================\n")
cat("Figure 4F-G: Collapsed Bubble Plots with Numbered Annotations\n")
cat("==============================================================================\n\n")
...
# -----------------------------------------------------------------------------
# Function to create collapsed bubble plot with numbered annotations
# -----------------------------------------------------------------------------

create_collapsed_bubble <- function(bubble_data, organism_name, org_color) {
    
    # Filter to significant terms matching keywords
    sig_keyword_data <- bubble_data %>%
        filter(p.adj < BUBBLE_P_ADJ_CUTOFF & !is.na(annotate_label)) %>%
        arrange(season_label, p.adj)
    
    # Assign numbers restarting for each season
    sig_keyword_data <- sig_keyword_data %>%
        group_by(season_label) %>%
        mutate(label_num = row_number()) %>%
        ungroup()
    
    cat("  Significant keyword terms to label:", nrow(sig_keyword_data), "\n")
    
    # Significance threshold line
    sig_line <- -log10(BUBBLE_P_ADJ_CUTOFF)
    
    # Create the bubble plot - use bubble_data for all points
    p_bubble <- ggplot(bubble_data, aes(x = x_value, y = neg_log10_padj)) +
        ...
        # Number labels - use sig_keyword_data directly for labeled points
        geom_text_repel(
            data = sig_keyword_data,
            aes(label = label_num),
            size = 3,
            fontface = "bold",
            max.overlaps = 50,
        ...
        # Facet by season only (rows)
        facet_grid(season_label ~ ., scales = "free") +
    ...
    # Create table data for each season
    create_season_table <- function(data, season_name) {
        table_data <- data %>%
            filter(season_label == season_name) %>%
            arrange(label_num) %>%
            mutate(
                `#` = label_num,
                `GO ID` = term,
                `GO Term` = str_trunc(go_name, 40)
            ) %>%
            select(`#`, `GO ID`, `GO Term`, division)
        
        return(table_data)
    }
    
    summer_table <- create_season_table(sig_keyword_data, "Summer")
    winter_table <- create_season_table(sig_keyword_data, "Winter")
    
    cat("  Summer table rows:", nrow(summer_table), "\n")
    cat("  Winter table rows:", nrow(winter_table), "\n")
    
    # Function to create a colored table grob
    create_table_grob <- function(table_data, title) {
        if (nrow(table_data) == 0) {
            return(textGrob(paste(title, ": No significant terms"), 
                           gp = gpar(fontsize = 10)))
        }
        
        # Create colors for each row based on division
        row_colors <- sapply(table_data$division, function(d) {
        ...
        display_data <- table_data %>% select(-division)
        
        # Create table theme
        tt <- ttheme_minimal(
            core = list(
                bg_params = list(fill = row_colors, col = "gray70"),
                fg_params = list(fontsize = 8, hjust = 0, x = 0.02)
            ),
            colhead = list(
                bg_params = list(fill = "gray90", col = "gray70"),
                fg_params = list(fontsize = 9, fontface = "bold", hjust = 0, x = 0.02)
            )
        )
        
        # Add title
        table_grob <- tableGrob(display_data, rows = NULL, theme = tt)
    ...
    summer_grob <- create_table_grob(summer_table, "Summer")
    winter_grob <- create_table_grob(winter_table, "Winter")
    
    # Combine tables vertically
    tables_combined <- arrangeGrob(
        summer_grob,
        winter_grob,
        ncol = 1,
        heights = c(0.5, 0.5)
    ...
    # Return components for flexible arrangement
    return(list(
        plot = p_bubble,
        tables = tables_combined,
        summer_table = summer_table,
        winter_table = winter_table
    ))
}

# -----------------------------------------------------------------------------
# Generate collapsed bubble plots
# -----------------------------------------------------------------------------

cat("Generating Host collapsed bubble plot (Fig4F)...\n")
host_collapsed <- create_collapsed_bubble(
    host_bubble_data, 
    "M. capitata (Host)", 
    host_color
)

cat("Generating Symbiont collapsed bubble plot (Fig4G)...\n")
symbiont_collapsed <- create_collapsed_bubble(
    symbiont_bubble_data, 
    "D. trenchii (Symbiont)", 
    symbiont_color
)
...
# -----------------------------------------------------------------------------
# Combine plot and tables side by side
# -----------------------------------------------------------------------------

# Host figure
fig4f_combined <- arrangeGrob(
    host_collapsed$plot,
    host_collapsed$tables,
    ncol = 2,
    widths = c(0.6, 0.4)
)

# Symbiont figure  
fig4g_combined <- arrangeGrob(
    symbiont_collapsed$plot,
    symbiont_collapsed$tables,
    ncol = 2,
    widths = c(0.6, 0.4)
)
...
ggsave("figures/Fig4F_bubble_collapsed_host.pdf", fig4f_combined, 
       width = 16, height = 10, dpi = 300)
ggsave("figures/Fig4F_bubble_collapsed_host.png", fig4f_combined, 
       width = 16, height = 10, dpi = 300)
cat("✓ Saved: Fig4F_bubble_collapsed_host.pdf/png\n")

ggsave("figures/Fig4G_bubble_collapsed_symbiont.pdf", fig4g_combined, 
       width = 16, height = 10, dpi = 300)
ggsave("figures/Fig4G_bubble_collapsed_symbiont.png", fig4g_combined, 
       width = 16, height = 10, dpi = 300)
cat("✓ Saved: Fig4G_bubble_collapsed_symbiont.pdf/png\n")
...
all_numbered_terms <- bind_rows(
    host_collapsed$summer_table %>% mutate(organism = "Host", season = "Summer"),
    host_collapsed$winter_table %>% mutate(organism = "Host", season = "Winter"),
    symbiont_collapsed$summer_table %>% mutate(organism = "Symbiont", season = "Summer"),
    symbiont_collapsed$winter_table %>% mutate(organism = "Symbiont", season = "Winter")
)

write.csv(all_numbered_terms, "data/bubble_plot_numbered_terms.csv", row.names = FALSE)
cat("✓ Saved: bubble_plot_numbered_terms.csv\n")

# Print summary
cat("\n=== Numbered Terms Summary ===\n")
cat("\nHost:\n")
cat("  Summer:", nrow(host_collapsed$summer_table), "terms\n")
cat("  Winter:", nrow(host_collapsed$winter_table), "terms\n")
cat("\nSymbiont:\n")
cat("  Summer:", nrow(symbiont_collapsed$summer_table), "terms\n")
cat("  Winter:", nrow(symbiont_collapsed$winter_table), "terms\n")

cat("\n✓ Figure 4F-G collapsed bubble plots complete!\n")
